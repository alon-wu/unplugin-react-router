...
| `layout` | `string` (v0.3) | **Declarative layout binding**: the top-level member this page belongs to is wrapped by `<layouts.dir>/<layout>.tsx` instead of the default shell. Only active while the `layouts` option is enabled; writing it without the option is inert. Layouts apply at *top-level member* granularity — every page of the same top-level directory block must agree (all undeclared → default shell; all the same layout → that layout). **Catch-all pages (`[...rest].tsx`) cannot declare `layout`** — they must stay in the default shell (a second global catch-all would never match) |
